...
BEAMNG_PATHS = [
    r"C:\Program Files\BeamNG.drive",
    r"C:\Program Files (x86)\BeamNG.drive",
    r"D:\Games\BeamNG.drive",
    os.path.expandvars(r"%USERPROFILE%\Documents\BeamNG.drive")
]

def find_beamng():
    for path in BEAMNG_PATHS:
        if os.path.exists(os.path.join(path, "Bin64", "BeamNG.drive.exe")):
            return path
        if os.path.exists(os.path.join(path, "BeamNG.drive.exe")):
            return path
    return None
...
def install_mod():
    ...
    if not beamng_path:
        messagebox.showerror("Ошибка", "BeamNG.drive не найдена!")
        return
...
'''
    ...
    messagebox.showinfo("Установка завершена", 
                       "🎵 Audio Optimizer успешно установлен!\n"
                       "Звук в BeamNG.drive улучшен.\n"
                       "FPS +10%.\n\n"
